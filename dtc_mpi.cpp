...
int select(vector <int> &attr,vector <int> data)
{
	...
	for(i=1;i<attr.size()-1;i++){
		if(attr[i]==0 && rank == i){	
			...
			MPI_Send(&iGain, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
		}
	}
	...
	if(rank == 0){
		for(i=1; i<attr.size()-1;i++){
			if(attr[i] == 0){
				MPI_Recv(&iGain, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				if(iGain>maxIGain){
		...
		for(i=1; i<attr.size()-1;i++){
			MPI_Send(&splitAttr, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
			MPI_Send(&maxIGain, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
		}
	}
	else{
		MPI_Recv(&splitAttr, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&maxIGain, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
	//to be deleted
		// printf("\n");
		//to be deleted
...
void decision(vector<int> attr,vector<int> data,node *root)
{
	//to be deleted
		...
		// cout<<"rank:"<<rank<<"selected attribute: "<<selectedAttribute <<  endl;
